...
`umockcall` is a module that encapsulates a `umock_c` call.
...
`umockcall_create` creates a new umock call instance.
...
`umockcall_destroy` frees resources associated with a umock call instance.
...
`umockcall_are_equal` checks if two call instances are equal.
...
`umockcall_stringify` produces the string representation for a umock call.
...
`umockcall_get_call_data` gets the typed call data portion from a umock call.
...
`umockcall_clone` clones a call instance.
...
`umockcall_set_fail_call` sets the flag indicating whether the call shall fail or not on the next negative test run.
...
`umockcall_get_fail_call` gets the flag indicating whether the call shall fail or not on the next negative test run.
...
`umockcall_set_ignore_all_calls` sets the flag indicating whether all calls with this name shall be ignored.
...
`umockcall_get_ignore_all_calls` gets the flag indicating whether all calls with this name shall be ignored.
...
`umockcall_set_call_can_fail` sets the flag indicating whether the call can fail on a subsequent negative tests run.
...
`umockcall_get_call_can_fail` gets the flag indicating whether the call can fail on a subsequent negative tests run.
